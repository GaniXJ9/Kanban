import clsx from "clsx";
import type { buttonProps } from "../../../features/types/common/button-props";
import SendIcon from "../../icons/send-icon";

const SendButton = ({
  text,
  size,
  fontSize,
  padding,
  rounded,
  onClick,
}: buttonProps) => {
  return (
    <button
      onClick={onClick}
      className={clsx(
        size,
        padding,
        `text-${fontSize}`,
        `rounded-${rounded}`,
        ` bg-[#6d94c7]  text-slate-200 lg:hover:bg-slate-200 lg:hover:border-slate-500 lg:hover:text-slate-500
        dark:bg-slate-700 dark:lg:hover:bg-slate-500 dark:lg:hover:text-slate-200
        font-normal lg:cursor-pointer transition-all duration-200`,
      )}
    >
      <SendIcon data-testid="btn-icon-id" />
      {text && <span>{text}</span>}
    </button>
  );
};

export default SendButton;
